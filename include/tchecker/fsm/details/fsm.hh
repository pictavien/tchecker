      ...
      public:
                                                 ...
                                                 tchecker::clock_constraint_container_t & invariant)
        {
          ...
          for (typename VLOC::loc_t const * loc : vloc)
            if (check_location_invariant(vloc, loc, intvars_val, invariant) == 0)
                                           ...
                                           tchecker::clock_constraint_container_t & tgt_invariant)
        {
          ...
          for (typename VLOC::loc_t const * loc : vloc)
            if (check_location_invariant(vloc, loc, intvars_val, src_invariant) == 0)
              return tchecker::STATE_INTVARS_SRC_INVARIANT_VIOLATED;
          ...
          for (auto it = begin; it != end; ++it)
            if (check_edge_guard(vloc, *it, intvars_val, guard) == 0)
              return tchecker::STATE_INTVARS_GUARD_VIOLATED;
          
          // apply statements
          for (auto it = begin; it != end; ++it)
            if (apply_edge_statement(vloc, *it, intvars_val, clkreset) == 0)
              return tchecker::STATE_INTVARS_STATEMENT_FAILED;
          
          // check target invariant
          for (typename VLOC::loc_t const * loc : vloc)
            if (check_location_invariant(vloc, loc, intvars_val, tgt_invariant) == 0)
