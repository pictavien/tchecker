...
location:Train$pid:Cross{invariant:x$pid<=5}
...
echo -n "property:crash{atomic:"
for t1 in `seq 1 $((N-1))`; do
    for t2 in `seq $((t1+1)) $N`; do
	test $t2 -ne 2 && echo -n " || "
	echo -n "Train${t1}.Cross && Train${t2}.Cross"
    done
done
echo " }"
